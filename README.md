...
To create a new project, use the Expo tool `create-react-native-app`.

New projects created this way will automatically have properly configured Apple TV targets created in their XCode projects, and will have the Android manifest correctly configured for Android TV.  New projects are set up to use the Expo CLI; the Expo dependency and `react-native.config.js` are included in the new app template.

```sh
# Make sure you have the app creation tool installed globally (this only needs to be done once on your system)
...
cd TestApp
# Now build and start the app in the tvOS Simulator - this will only work on a macOS machine. This command can also be run via "yarn tvos".
npx expo run:ios --scheme MyApp-tvOS --device "Apple TV"
# You can also build and start the app on an iOS phone simulator. This command can also be run via "yarn ios".
npx expo run:ios
# or specify a simulator:
npx expo run:ios --scheme MyApp --device "iPhone 15"
# This command builds and starts the app in an Android TV emulator or a phone emulator (needs to be created in advance). This command can also be run via "yarn android".
