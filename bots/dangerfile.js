...
if (!isMergeRefMaster && isMergeRefStable) {
  ...
  const idea =
    'The base branch for this PR is something other than `master`. [Are you sure you want to target something other than the `master` branch?](http://reactnative.dev/docs/contributing.html#pull-requests)';
  fail(`${title} - <i>${idea}</i>`);
}
